...
BEGIN
    ...
    SELECT
        ...
        --pc.Purchase_Date,-------
        --pc.Ship_Date,
		REPLACE(pc.Purchase_Date, '-', '') AS Purchase_Date_No_Hyphen,
        REPLACE(
        CASE 
            WHEN pc.Ship_Date = 'N/A' THEN 
                CAST(FLOOR(RAND() * (1800 - 1700) + 1700) AS VARCHAR(4)) + 
                RIGHT('00' + CAST(FLOOR(RAND() * 12 + 1) AS VARCHAR(2)), 2) + 
                RIGHT('00' + CAST(FLOOR(RAND() * 31 + 1) AS VARCHAR(2)), 2)
            ELSE pc.Ship_Date
        END,
        '-', ''
    ) AS Ship_Date_No_Hyphen_Updated
